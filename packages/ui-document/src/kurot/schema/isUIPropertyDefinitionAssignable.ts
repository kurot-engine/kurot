import type { UIPropertyDefinition } from './UIComponentDefinition.js';
...
export function isUIPropertyDefinitionAssignable(source: UIPropertyDefinition, target: UIPropertyDefinition): boolean {
	...
	if (source.enumValues !== undefined) {
		return source.enumValues.every(value => matchesUIPropertyDefinition(value, target));
	}
...
function numericDomainIsAssignable(source: UIPropertyDefinition, target: UIPropertyDefinition): boolean {
