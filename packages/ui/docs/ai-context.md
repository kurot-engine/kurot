...
| `skinParts`                                        | `Skin.skinParts: string[]` lists the complete named part set exposed by a skin. `Component` collects the values into one internal read-only map, exposes it to subclasses through typed `skinParts`, then invokes `onSkinReady()`. Before replacement or removal, `onSkinRemoved()` runs while the old complete map remains available. No dynamic properties are written to the component instance.                                                                            | `components/Skin.ts`, `components/Component.ts`                                         |
