...
export function uploadCustomUniforms(
	...
	outputWidth: number,
	outputHeight: number,
	inputs: Record<string, FilterSampler>,
): void {
	const locations = program.uniforms;
	const automaticTypes: Record<string, number> = {
		projectionVector: gl.FLOAT_VEC2,
		uSampler: gl.SAMPLER_2D,
		uTextureSize: gl.FLOAT_VEC2,
		uInputSize: gl.FLOAT_VEC4,
		uInputClamp: gl.FLOAT_VEC4,
		uResolution: gl.FLOAT,
		uOutputSize: gl.FLOAT_VEC4,
	};
	...
	if (locations.uTextureSize) {
		gl.uniform2f(locations.uTextureSize, width, height);
	}
	if (locations.uInputSize) {
		gl.uniform4f(locations.uInputSize, width, height, 1 / width, 1 / height);
	}
	...
	if (locations.uResolution) {
		gl.uniform1f(locations.uResolution, resolution);
	}
	if (locations.uOutputSize) {
		gl.uniform4f(locations.uOutputSize, outputWidth, outputHeight, 1 / outputWidth, 1 / outputHeight);
	}
	...
	for (const [name, info] of Object.entries(program.uniformInfo)) {
		...
		if (
			resources[name] ||
			(name.endsWith('Matrix') && resources[name.slice(0, -6)]) ||
			(name.endsWith('Clamp') && resources[name.slice(0, -5)])
		)
			continue;
...
function uploadUniform(
	...
	if (
		...
		throw new TypeError(
			`CustomFilter uniform ${name} requires ${count * size} finite ${integer ? 'integer' : 'numeric'} values.`,
		);
	}
	if (integer) {
