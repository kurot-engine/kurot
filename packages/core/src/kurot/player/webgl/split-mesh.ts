...
export function* splitMesh(vertices: readonly number[], uvs: readonly number[], indices: readonly number[]): Generator<MeshChunk> {
	...
	for (let i = 0; i + 2 < indices.length; i += 3) {
		...
		if (mapped.size + added > WebGLVertexArrayObject.MAX_VERTICES
