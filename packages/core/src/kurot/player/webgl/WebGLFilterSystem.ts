...
export class WebGLFilterSystem {
	...
	public render(
		...
		resolution: number,
		composite: (output: WebGLRenderTarget) => void,
	): void {
		...
		try {
			...
			for (const target of this._owned) {
	...
	private apply(
		...
		if (filter instanceof MultiPassFilter) {
			...
			for (const [index, pass] of filter.passes.entries()) {
				...
				for (const ref of [pass.input!, ...Object.values(pass.textures ?? {})]) {
					if (ref !== 'original' && --uses[ref] === 0) {
						this.release(outputs[ref]);
					}
				}
				if (uses[index] === 0) {
					this.release(outputs[index]);
				}
			}
		...
		this._context.$drawFilterPass(
			filter instanceof BlurFilter ? undefined : filter,
			input,
			output,
			resolution,
			textures,
	...
	private blur(
		...
		if (
			source.width !== Math.max(1, Math.round(width * resolution)) ||
			source.height !== Math.max(1, Math.round(height * resolution))
		) {
